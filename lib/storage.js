export const Storage = {
    ...
    ensureDataIntegrity(data) {
        ...
        console.log('ensureDataIntegrity - students from data:', students.length);
    ...
    save(data) {
        try {
            ...
            console.log('Storage.save called with students:', data.students?.length);
            
            // Ensure data integrity before saving
            const dataToSave = this.ensureDataIntegrity(data);
            console.log('Storage.save after integrity, students:', dataToSave.students?.length);
            ...
            try {
                ...
                console.log('Storage.save completed, students:', dataToSave.students?.length);
                
                // Verify it was saved correctly
                const verify = JSON.parse(storage.getItem(this.key));
                console.log('Verified storage, students:', verify?.students?.length);
            } catch (storageErr) {
                console.error('Storage error (可能是大小限制):', storageErr.message);
                // Try saving with reduced data if too large
                if (storageErr.name === 'QuotaExceededError') {
                    console.log('Attempting to save with reduced data...');
                    const reducedData = {
                        ...dataToSave,
                    ...
                    console.log('Saved with reduced data');
                }
            }
        } catch (e) {
    ...
    replaceWithGoogleData(existingData, googleData) {
        console.log('🔄 replaceWithGoogleData called');
        console.log('   existingData students:', existingData?.students?.length);
        console.log('   googleData received:', !!googleData);
        
        if (!googleData) {
            console.log('   ⚠️ No googleData, returning existingData');
            return existingData;
        }
        
        // Get all students from Google
        const googleStudents = googleData.students || [];
        console.log('   📊 Google students raw count:', googleStudents.length);
        ...
        console.log('Google students after processing:', uniqueStudents.length);
        ...
        console.log('   ✅ replaceWithGoogleData returning:', { 
